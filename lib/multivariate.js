 ...
 *                              is false.
 *
 *        {Number} cookieExpireDays The number of days the cookie, which maps a user
 *                                  to a certain test, should persist in the user's
 *                                  browser. The default value is 90.
 *
...
function Multivariate(testName, options) {
    ...
    if (options) {
        ...
        this.cookieExpireDays = options.cookieExpireDays ?
            options.cookieExpireDays : 90;
    }
    ...
    /**
     * Returns true if the current user is in the test group, false if the user
     * is in the control group (not in test).
     *
     * @return {Boolean} true if the user is in the test group, false otherwise
     */
    this.isInTestGroup = function() {
        ...
        this.isInTest = getCookieValue(this.testName);
        // if not...
        if (this.isInTest === undefined || this.isInTest === null) {
            /*
             ...
             * sample size results in always false for test group (all control
             * group).
             *
             * The max generated value is 99.99..., and 99.99... < 100 == true.
             * Meaning a 100 sample size value results in always true for test
             * group (none in control group).
             */
            this.isInTest = (Math.random() * 100) < this.sample;
            debug("setting user to isInTest => " + this.isInTest);

            // remember this value for later
            createCookie(this.testName, this.isInTest, this.cookieExpireDays);
        } else {
            // make sure to convert from string to boolean
            this.isInTest = (this.isInTest === "true");
            debug("cookie found, user in isInTest: " + this.isInTest);
        }
    ...
    /**
     * Sends Google Analytics a message to track this page. Note that this is
     * called automatically when <code>runTest()</code> is called, if Google
     * Analytics support is enabled.
     */
